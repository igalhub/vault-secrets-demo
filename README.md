...
| **Linux VM (Proxmox home lab)** | ✅ Tested | Deployed on Ubuntu Server 24.04.3 running as a Proxmox VE 9.2.3 VM on a Beelink SER mini PC. Clean deployment with no issues beyond the standard bind-mount permission fix (`sudo chown 100 vault/data`). See `docs/HOMELAB_DEPLOYMENT.md` for the full walkthrough. |
...
| **Self-hosted (Proxmox home lab)** | ✅ Tested | Runs identically to local Docker deployment. No cloud account required. Static VM IP replaces `localhost` for network access. See `docs/HOMELAB_DEPLOYMENT.md`. |
